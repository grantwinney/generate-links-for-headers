...
headers.forEach(function(header) {
    var id = getFirstId(header);
    if (id !== undefined) {
        var link = '<a onclick="copyHeaderLinkToClipboard(event, \'' + baseUrl + id + '\')" href="javascript:;" title="Copy link to clipboard"><svg height="16" viewBox="0 0 16 16" width="16"><path fill-rule="evenodd" d="' + chainsvg + '"></path></svg></a>';
        header.innerHTML = '<div class="glfh_headerContainer"><div class="glfh_linkContainer">' + link + '</div>' + header.innerHTML + '</div>';
    }
});
